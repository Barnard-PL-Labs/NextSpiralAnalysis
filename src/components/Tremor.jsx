...
const TremorPolarPlot = ({ result }) => {
    ...
    const anisotropy = result["R-L hemi-pr"] === "Inf" ? 1 : parseFloat(result["R-L hemi-pr"]) || 0;
    ...
    return (
        <div style={{ width: '300px', height: '300px', textAlign: 'center', color: 'white' }}>
            {hasAxes ? (
                <Plot data={plotData} layout={layout} config={{displayModeBar:false, responsive:true,}} style={{ width: '100%', height: '100%' }} />
            ) : (
            ...
            {hasAxes && (
                <div style={{ color: 'cyan', fontSize: '12px', textAlign: 'left', marginTop: '10px' }}>
                    ...
                    <p style={{ color: 'red', margin: 0 }}>Anisotropy: {anisotropy.toFixed(3)}</p>
                </div>
            )}
        </div>
    );
